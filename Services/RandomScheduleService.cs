...
namespace SportsScheduleProLibrary.Services
{
    public class RandomScheduleService
    {
        ...
        public static List<Game> GameScheduleV2(League league = null, Season season = null, Club club = null)
        {
            ...
            foreach (Club c in clubs)
            {
                ...
                else
                {
                    foreach (Field f in fieldsForClub)
                    {
                        ...
                        while (currentDate <= currentLeagueSeason.EndDate)
                        {
                            ...
                            {
                                if (!tzi.IsDaylightSavingTime(currentDate))
                                {
                                    for (int x = 0; x < f.DailyGamesPerFieldSaturday+1; x++)
                                    {
                                        possibleUnfilteredTimeSlots.Add(new Tuple<Field, DateTime>(f, currentDate.AddHours(f.EarliestGameTimeHourSaturday-1).AddMinutes(f.EarliestGameTimeMinuteSaturday+30).AddMinutes(f.FieldGameLengthWindow * x)));
                                    }
                                }
                                else
                                {
                ...
                foreach (League l in leagues)
                {
                    ...
                    foreach (Team t in l.Teams.OrderByDescending(s => s.ExcludedGameDates.Count()).OrderBy(_ => rng.Next()))
                    {
                        foreach (Team u in l.Teams.OrderByDescending(s => s.ExcludedGameDates.Count()).OrderBy(_ => rng.Next()))
                        {
                    ...
                    foreach (Team t in l.Teams.OrderByDescending(s => s.ExcludedGameDates.Count()).OrderBy(_ => rng.Next()))
                    {
                        ...
                        while (teamMatchingCount.Where(s => s.Key.HomeTeamId == t.TeamId || s.Key.AwayTeamId == t.TeamId).Sum(s => s.Value) < l.GamesPerSeason)
                        {
                            ...
                            if (teamMatchingCount.Where(s => s.Key.HomeTeamId == t.TeamId || s.Key.AwayTeamId == t.TeamId).Sum(s => s.Value) >= l.GamesPerSeason)
                                continue;
                            Tuple<Field, DateTime> timeSlot = possibleUnfilteredTimeSlots.Where(s => t.League.Fields.Contains(s.Item1) && CheckCoachAvailable(s, tm.AwayTeamId, tm.HomeTeamId, dbc))
                                .OrderByDescending(s => CheckDaysWithoutGame(s, tm.AwayTeamId, tm.HomeTeamId, dbc))
        ...
        private static bool CheckDaysWithoutGame(Tuple<Field, DateTime> schDate, int awayTeamId, int homeTeamId, SportsScheduleProDataContext dbc)
        {
            if (dbc.Games.Where(s => (s.HomeTeamId == awayTeamId || s.AwayTeamId == awayTeamId || s.HomeTeamId == homeTeamId || s.AwayTeamId == homeTeamId) && schDate.Item2.Date == s.ChosenScheduleTime.Date).Count() > 0)
                return false;
            else
                return true;
        ...
        private static bool CheckCoachAvailable(Tuple<Field, DateTime> startTime, int taId, int thId, SportsScheduleProDataContext dbc)
        {
            if(dbc == null)
            ...
            return games.Count(s => startTime.Item2 < s.ChosenScheduleTime.AddMinutes(startTime.Item1.FieldGameLengthWindow) && s.ChosenScheduleTime < startTime.Item2.AddMinutes(startTime.Item1.FieldGameLengthWindow) && s.Field == startTime.Item1) == 0;
        }

//        public static List<Game> GameSchedule(League league = null, Season season = null, Club club = null, int tries = 0)
//        {
...
//                                availableForTeams.RemoveAll(r => dbc.Games.Include(s => s.Field).Where(s => ((s.HomeTeamId == g.HomeTeamId || s.HomeTeamId == g.AwayTeamId) && (s.AwayTeamId == g.AwayTeamId || s.AwayTeamId == g.HomeTeamId)) && r.Item2 > s.ChosenScheduleTime.AddDays(-8) && r.Item2 < s.ChosenScheduleTime.AddDays(Math.Max(0, -7 + (tries / 20)))).Count() > 0);
